...
#define NARRAY 500   /* array size */
#define NBUCKET 5   /* bucket size */
#define INTERVAL 100 /* bucket range */
...
//    for (i = 0; i < NBUCKET/2; ++i)
//    {
//        buckets[i] = InsertionSort(buckets[i]);
//    }
    //clock_t begin = clock();
	for(i=0 ; i < NBUCKET / NUM_THREADS ; ++i)
	{
		for(j=0 ; j < NUM_THREADS ; ++j)
		{
			pthread_create(&threads[j], NULL , InsertionSort , buckets[j+NUM_THREADS*i]);
		}
		for(j=0 ; j < NUM_THREADS ; ++j)
		{
            pthread_join(threads[j],&buckets[j+NUM_THREADS*i]);
		}
	}
    //clock_t end = clock();
...
struct Node *InsertionSort(struct Node *list)
{
    ...
    pthread_exit((struct Node*)nodeList);
    return nodeList;
}
